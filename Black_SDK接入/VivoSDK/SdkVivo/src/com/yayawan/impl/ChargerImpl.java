...
public class ChargerImpl implements YYWCharger {
	        ...
	        http.post("http://union.yayawan.com/pay_handler", params, new StringResponseHandler() {
	            ...
	            public void onResponse(String content, URL url, int code) {
	            	...
	            	try {
	                    ...
	                    orderNumber = orderString.optString("order_id", "");
	                    accessKey = orderString.optString("accessKey", "");
	            	...
	            	if (code != 200) {
	            		Toast.makeText(paramActivity,"获取订单失败", Toast.LENGTH_SHORT).show();
					}else {
						...
						new Handler(Looper.getMainLooper()).post(new Runnable() {
				            ...
				            @Override
				            public void run() {
				            	pay_run(paramActivity, orderNumber, accessKey);
				            }
				        });
	...
	private void pay_run(final Activity paramActivity, String orderNumber, String accessKey) {

		YaYawanconstants.pay(paramActivity, orderId,orderNumber,accessKey);
