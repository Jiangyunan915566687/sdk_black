...
public class YaYawanconstants {
	...
	public static void setRoleData(Activity paramActivity) {
		Log.d("black", "调用了setRoleData()1");
		if (YYWMain.mRole != null) {
			Log.d("black", "调用了setRoleData()2");
			/*
			try {
	...
	public static void setData(Activity activity, String roleId,
		...
		//if (YYWMain.mRole != null) {
			try {				
				...
				if(DeviceUtil.getGameInfo(mActivity, "isAddTime").equals("yes")){
					//1462050000
					if(Long.parseLong(roleCTime) < 1462050000){
						Log.d(TAG, "进来了 yywTime(roleCTime)");
						roleCTime = yywTime(roleId);
					}else{
						//如果时间比较正常
						Log.d(TAG, "进来了 userTime(String roleId,String RoleCTime)");
						roleCTime = userTime(roleId, roleCTime);
					}
				}
				...
				if(TextUtils.isEmpty(roleId)||TextUtils.isEmpty(roleName)||
						roleId.equals(roleName)){
					Log.d("black", "roleId & roleName == null 选择服务器时机  跳出上传");
					return ;
				}
				UCGameSDK.defaultSDK().submitExtendData("loginGameRole",
						jsonExData);
	...
	/**
	 * 用游戏id 生成 一个创建游戏角色的时间 记录在本地  
	 * 每个角色对应一个创建游戏角色的时间
	 * @param roleId 传进游戏角色的id
	 * @return
	 */
	private static String yywTime(String roleId){
		SharedPreferences sp = mActivity.getSharedPreferences("usertime", Context.MODE_PRIVATE);
		String newRoleCTime = "";
		newRoleCTime = sp.getString(roleId,System.currentTimeMillis()/1000+"");
		sp.edit().putString(roleId, newRoleCTime).commit();
		return newRoleCTime;
	}
	private static String userTime(String roleId,String RoleCTime){
		SharedPreferences sp = mActivity.getSharedPreferences("usertime", Context.MODE_PRIVATE);
		String newRoleCTime = "";
		newRoleCTime = sp.getString(roleId,RoleCTime);
		sp.edit().putString(roleId, newRoleCTime).commit();
		return newRoleCTime;	
	}
	/**
