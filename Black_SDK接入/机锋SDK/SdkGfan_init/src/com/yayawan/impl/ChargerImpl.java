...
public class ChargerImpl implements YYWCharger {
    ...
    private void pay_run(final Activity paramActivity) {
    	...
    	Order order2 = new Order(YYWMain.mOrder.goods, YYWMain.mOrder.goods, Integer.valueOf(YYWMain.mOrder.money.toString()) / 10, orderId);

        GfanPay.getInstance(paramActivity).pay(order2,
                new GfanPayCallback() {

                    @Override
                    public void onSuccess(User user, Order jfOrder) {

                        YYWMain.mOrder.id = jfOrder.getNumber();
